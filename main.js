...
console.log(this.$refs.tree2.getCurrentKey())
        ...
        $('#commitreflash').click(() => {
            ...
            players = [];
            if (location.href.match(/\/live\/foucs\/stream\/audit/g)) {
                setTimeout(() => {
                    urls = [];
                    rander(urls);
                }, 888)
                ...
                const freshTest = setInterval(() => { //检测是否刷新完毕
                    if ($('object').length && $('object').eq(0).parent().next().children().first().attr('onclick').split('"')[5] !== flag1 && $('object').eq(1).parent().next().children().first().attr('onclick').split('"')[5] !== flag2 && $('object').eq(2).parent().next().children().first().attr('onclick').split('"')[5] !== flag3) { //执行完
                        urls = [];
                        rander(urls);
        ...
        //let superviceStartTime = $('#superviceStartTime').val();
        //console.log(superviceStartTime)
        //$('body').html('');
        ...
        //    $('body').html(`
        //        <div id="app">
        //            <div class="vueItem" v-for="(item, index) in list" :key="index" :url="item.streamUrl">
        //                <video :id="'video' + index" controls muted></video>
        //                {{item.name}}
        //            </div>
        //            <button style="position:fixed;left:50%;bottom:0;" @click="sumAndRefresh">换一批</button>
        //        </div>
        //        <script>
        //            new Vue({
        //                el: "#app",
        ...
        //                    superviceTime: 1,
        //                    list: [],
        //                    hosts: ['http://pili-live-rtmp.hzwangjiao.com/', 'http://pili-live-rtmp.bingo28.cn/', 'http://pili-live-rtmp.shelinkme.cn/', 'http://pili-live-rtmp.gamepk8.cn/', 'http://pili-live-rtmp.xingketech.com/'],
        //                    flvList: []
        //                },
        //               methods: {
        //                    getList (pageType) {
        //                        axios.post('/live/normal/stream/audit/getLiveNormalStreamList', {
        //                            pageSize: this.pageSize,
        //                            pageType: pageType,
        //                            uproomids: this.uproomids,
        //                            uproomUserids: this.uproomUserids,
        //                            superviceStartTime: this.superviceStartTime,
        //                            superviceTime: this.superviceTime
        //                        }).then((res) => {
        //                            let data = JSON.parse(res.data.replace('(', '').replace(')', ''));
        //                            let list = []
        //                            this.audituserCounts = data.audituserCounts;
        //                            this.flashCounts = data.flashCounts;
        //                            this.flashTimeAverAge = data.audituserCounts / data.flashCounts;
        //                            this.list = data.list;
        //                            this.superviceStartTime = data.superviceStartTime;
        //                            this.uproomUserids = data.uproomUserids;
        //                            this.uproomids = data.uproomids;
        //                            data = null;
        //                            let hostIndex = 0;
        //                            if (this.flvList.length) {
        //                                this.flvList.forEach((e) => {
        //                                    e.destroy();
        //                                })
        //                                this.flvList = [];
        //                            }
        //                            this.$nextTick(function () {
        //                                $('.vueItem video').each((i, e) => {
        //                                    e.onended = () => {
        //                                        $(e).replaceWith('<span class="ended">直播结束了</span>')
        //                                    }
        //                                    const flv = flvjs.createPlayer({
        //                                        type: 'flv',
        //                                        url: this.hosts[hostIndex] + this.list[i].streamUrl.split('hzwangjiao.com/')[1] + '.flv',
        //                                        isLive: true,
        //                                    }, {
        //                                        fixAudioTimestampGap: false,
        //                                        enableStashBuffer: false,
        //                                    });
        //                                    flv.attachMediaElement(e);
        //                                    flv.load();
        //                                    flv.play();
        //                                    this.flvList.push(flv);
        //                                    if (i % 6 == 0 && i !== 0) {
        //                                        hostIndex ++;
        //                                    }
        //                                })
        ...
        //    `);
        //}
        //const waitingReady = setInterval(() => {
        //    if (window.Vue && window.axios && window.ELEMENT && window.flvjs){
        //        rander();
        //        console.log("okOKOKO");
