...
(function($) {
        ...
        //进审提醒功能
        const $refresherInsert = bool => { //插入自定刷新器
            $(
            ...
            let timer = null;
            $(".refresh").on("click", "button", function() { //定义自定刷新器点击事件
            ...
            if (document.cookie.match(/time=\d+/g)) { //刷新后根据设定值继续刷新
                const time = parseInt(
        ...
        const setSize = width => { //设置直播截屏图片尺寸
            let bite = 1.8;
        ...
        const setButtonSize = size => { //设置直播截屏按钮尺寸
            $("table input:visible")
                .css({
                    width: size / 3 + "px",
                    padding: "0",
        ...
        const setTitleSize = size => { //设置直播截屏昵称尺寸
        ...
        const cancelAnimate = () => { //取消动画功能
            $(":animated").stop(true, true);
        };
        const retoperateEmpasisUser = response => { //推流成功后取消推流弹窗记录审核记录
        ...
        const Ajax = { //去掉兼容的Ajax对象
            xmlhttp: function() {
                return new XMLHttpRequest();
            }
        };
        Ajax.Request = function() { //Ajax的请求方法
        ...
        const addEmpasisUser = (roomid, userid) => { //推流
        ...
        const retoperateListOne = response => { //重写叉掉重点流的回调函数
                var ret = eval("(" + response.responseText + ")");
                var oldrid = ret.roomid; //得到回执参数
                var uid = ret.userid;
                ...
                var nowuid1 = "";
                var nowuid2 = "";
                const time = +new Date() //页面内容绑定
                var table = ""; //循环列表
                for (var i = 0; i < ret.list.length; i++) {
                    var obj = ret.list[i]; //绑定视频数据
                    nowuid = obj.userid;
        ...
        <table style="float: left; width: 153px; margin-right: 0px; margin-bottom: 0px; outline: none;" border="0" height="305px" cellspacing="1" cellpadding="0"  class="content" width="210px" border="0">
            <tr>
                <td style="padding: 0px;">
                    <div id = "video_${obj.userid}_${obj.roomid}" class=${time}></div>
                </td>
            </tr>
            <tr>
                <td style="padding: 0px;">
    ...
    obj.roomid
  }">待定 <img class="full" src="images/ok.gif" title="全屏" width="18" height="18" style="float: right; cursor: pointer;"><img class="sound" src="images/voice.png" width="18" height="18" style="float: right; margin: 0 8px 0 0; cursor: pointer;">
                </td>
            </tr>
        </table>
        `;
            ...
            <tr>
                <td style="padding: 0px;">
                    <div id = "video_${obj.userid1}_${obj.roomid}" class=${time + `1`}></div>
                </td>
            </tr>
            <tr>
                <td style="padding: 0px;">
      ...
      obj.roomid
            ...
            <tr>
                <td style="padding: 0px;">
                    <div id = "video_${obj.userid2}_${obj.roomid}" class=${time + `1`}></div>
                </td>
            </tr>
            <tr>
                <td style="padding: 0px;">
      ...
      obj.roomid
        ...
        $("#div_" + uid + "_" + oldrid)
            ...
            .css("outline", "none");
        let activeIndex = 0;
        $('div[id^=div_]').each((i, e) => {
            if (e.id === "div_" + uid + "_" + oldrid) {
                activeIndex = i
        ...
        if (
            ...
            $(`.${time}`).css('position', 'relative').css('height', '272px').css('overflow', 'hidden').html(`<video id="video${time}" controls muted autoplay preload="auto" style="width: 154px; height: 272px;"></video>`)
            let src = 'http:' + stream_url.match(/\/\/pili-live-rtmp.hzwangjiao.com\/fuliao-live\/\d+/g)[0] + '.m3u8'
            setTimeout(() => {
                videojs(`video${time}`, {
                    language: 'zh-CN'
        ...
        $('.sound').eq(activeIndex).click((e) => {
            $(e.target).toggleClass('sounded')
            $(e.target).parentsUntil('table').find('.vjs-mute-control').first().click()
        })
        $('.full').eq(activeIndex).click((e) => {
            $('#fullvideo').show()
            videojs('video' + time).pause()
            videojs('video', {}, function onPlayerReady() {
                this.src({
                    src: 'http:' + stream_url.match(/\/\/pili-live-rtmp.hzwangjiao.com\/fuliao-live\/\d+/g)[0] + '.m3u8',
                    type: "application/x-mpegURL"
                })
            })
        })
    ...
    `).appendTo('head'); //插入video.js
        $('<div id="fullvideo" style="display: none; position: fixed; z-index:1;  width: 100vw; height: 100vh; left: 0; top: 0; background-color: rgba(0, 0, 0, .6);"><video id="video" autoplay preload="auto" style="height: 100vh;width: 50%; margin: 0 25%;"></video><span id="close-video" style="position: absolute; width: 100px; height: 100px; text-align: center; line-height: 85px; border-radius: 50%; background-color: rgba(0, 0, 0, .6); font-size: 100px; color: #fff; top: 0; right: 20px; z-index: 1000; cursor: pointer;">×</span></wrapper>').appendTo('body')
        $('<style>.sounded { outline: 2px solid #f40; }</style>').appendTo('head')
        const urls = $('head').html().match(/\/\/pili-live-rtmp.hzwangjiao.com\/fuliao-live\/\d+/g) //获取当前页面视频链接
        if ($('.content_div table').first().find('table').length) { //矫正置顶方和收费方顺序
            for (let i = 0; i < $('.content_div table').first().find('table').length; i++) {
                urls.unshift(urls.pop())
            }
        }
        let index = 0;
        $('.content_div div[id^="video"]').css('position', 'relative').css('height', '272px').css('overflow', 'hidden').each((i, e) => { //替换flash为video
            let str = `
                      <video id="live${i}" controls muted autoplay preload="auto" style="width: ${width}px; height: ${height}px;"></video>
                  `
            $(e).html(str)
        })
        const vueTest = setInterval(() => { //检测video.js是否执行完毕
            if (window.videojs) { //执行完毕
                videoLanguage()
                $('.content_div video').each((i, e) => { //操作video
                ...
                clearInterval(vueTest) //取消测试video.js
            }
        }, 100)
        ...
        setTimeout(() => {
            ...
            $('.content_div>table div[id^="div"]>table tr:nth-child(5) td').append('<img class="full" src="images/ok.gif" title="全屏" width="18" height="18" style="float: right; cursor: pointer;"><img class="sound" src="images/voice.png" width="18" height="18" style="float: right; margin: 0 8px 0 0; cursor: pointer;">')
            $('.sound').click((e) => {
                $(e.target).toggleClass('sounded')
                $(e.target).parentsUntil('table').find('.vjs-mute-control').first().click()
            })
            $('.full').each((i, e) => {
                ((i, e) => {
                    $(e).click((e) => {
                        $('#fullvideo').show()
                        index = $(e.target).parentsUntil('table').find('video')[0].id.match(/\d+/)[0]
                        console.log(index)
                        videojs('live' + index).pause()
                        videojs('video', {}, function onPlayerReady() {
                            this.src({
                                src: 'http:' + urls[index] + '.m3u8',
                                type: "application/x-mpegURL"
                            })
                        })
                    })
            ...
            $('#close-video').click(() => {
                $('#fullvideo').hide()
                console.log($('#fullvideo video')[0].pause())
                //videojs('video').pause()
                $('table video').each((i, e) => {
                    e.play()
                })
            })
        }, 300);
    ...
    `).appendTo('head'); //插入video.js
        ...
        let index = 0
        let width = 132;
        let height = width * 1.77; //设置视频容器宽高
        $('.content_div div[id^="video"]').css('position', 'relative').css('height', '211px').css('overflow', 'hidden').each((i, e) => { //替换flash为video
            let str = `
                      <video id="live${i}" controls muted autoplay preload="auto" style="width: ${width}px; height: ${height}px;"></video>
                  `
            $(e).html(str)
        })
        const videoJsTest = setInterval(() => { //检测video.js是否执行完毕
            if (window.videojs) { //执行完毕
                videoLanguage()
                $('.content_div video').each((i, e) => { //操作video
                    let src = 'http:' + urls[i] + '.m3u8'
        ...
        setTimeout(() => {
            ...
            $('font:even').next().css({
                ...
                'margin-right': '2px',
                cursor: 'pointer'
            }).before('<img class="full" src="images/ok.gif" title="全屏" width="16" height="16" style="float: right; cursor: pointer;">').click((e) => {
                $(e.target).toggleClass('sounded')
                $(e.target).parentsUntil('table').find('.vjs-mute-control').first().click()
            })
            $('.full').each((i, e) => {
    ...
    `).appendTo('head'); //插入video.js
        ...
        const urls = $('head').html().match(/\/\/pili-live-rtmp.hzwangjiao.com\/fuliao-live\/\d+/g) //获取当前页面视频链接
        let index = 0;
        $('div[id^="video"]').css('position', 'relative').css('height', '272px').css('overflow', 'hidden').each((i, e) => { //替换flash为video
            let str = `
                      <video id="live${i}" controls muted autoplay preload="auto" style="width: ${width}px; height: ${height}px;"></video>
                  `
            $(e).html(str)
        ...
        const vueTest = setInterval(() => { //检测video.js是否执行完毕
            if (window.videojs) { //执行完毕
                videoLanguage()
                $('div[id^="video"] video').each((i, e) => { //操作video
                    let src = 'http:' + urls[i] + '.m3u8'
                    videojs('live' + i, {
                        language: 'zh-CN'
                    }, function onPlayerReady() {
        ...
        setTimeout(() => {
            ...
            $('div[id^="div"]>table').each(function (i, e) {
                ...
                $(e).find("tr:nth-child(2) td").append('<img class="full" src="images/ok.gif" title="全屏" width="20" height="20" style="float: right; cursor: pointer;"><img class="sound" src="images/voice.png" width="18" height="17" style="float: right; margin: 2px 2px 0 0; cursor: pointer;">')
                    .find('a').css({
                        display: 'inline-block',
                        width: '50px',
                        'height': '16px',
                        ...
                        $(e).parentsUntil('table').attr('title', e.innerHTML)
                    })
            });
            $('.sound').click((e) => {
                $(e.target).toggleClass('sounded')
        ...
        $('<link href="https://unpkg.com/video.js/dist/video-js.css" rel="stylesheet"><script src="https://unpkg.com/video.js/dist/video.min.js"></script><script src="https://unpkg.com/videojs-contrib-hls/dist/videojs-contrib-hls.min.js"></script>').appendTo('head');
