...
stream_url != null
			...
			console.log(src)
			setTimeout(() => {
					videojs(`video${time}`, { language: 'zh-CN' }, function onPlayerReady(){
	...
	const vueTest = setInterval(() => {  //检测video.js是否执行完毕
			if (window.videojs) {  //执行完毕
					...
					$('.content_div video').each((i, e) => {  //操作video
							...
							videojs('live' + i, { language: 'zh-CN' }, function onPlayerReady(){
									this.src({src, type: "application/x-mpegURL"})
							})
					})
					...
					((i, e) => {
							$(e).click((e) => {
									...
									videojs('video', {}, function onPlayerReady(){
											this.src({src: 'http:' + urls[index] + '.m3u8', type: "application/x-mpegURL"})
									})
							})
					})(i, e)
			})
...
if (document.readyState == "complete") {
	...
	if (
		...
		document.cookie = "size=131;db=0;hd=0;rp=0;refreshTimes=0;demon=0;total=0;";
		$("#autInput").val(131);
		setSize(131);
		setButtonSize(130);
		setTitleSize(130);
...
$(".content_div>div>table").on("mouseenter", "table", function () {
	...
	$(".sex").click(function () {
		$(this)
			...
			$('font:even').next().css({ float: 'right', 'margin-right': '2px', cursor: 'pointer' }).before('<img class="full" src="images/ok.gif" title="全屏" width="16" height="16" style="float: right; cursor: pointer;">').click((e) => {
					$(e.target).toggleClass('sounded')
					$(e.target).parentsUntil('table').find('.vjs-mute-control').first().click()
			})
			$('.full').each((i, e) => {
		...
		$(e)
			...
			.append(
				'<img style="width:14px;height:14px;" src="/images/link.png">'
					...
					.find('a').css({ display: 'inline-block', width: '50px', 'height': '16px', overflow: 'hidden' }).each((i, e) => {
